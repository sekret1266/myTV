...
def main():
    ...
    # ИСПРАВЛЕНО: Правильные отступы и инициализация заголовка плейлиста
    playlist_content = '#EXTM3U x-tvg-url="https://iptvx.one/EPG7"\n'
    links_found = 0

    # ИСПРАВЛЕНО: Заменен if на for. Теперь скрипт корректно обработает и 1 канал, и 10.
    for ch in channels:
        ...
        if live_link:
            ...
            print(f"ОШИБКА: Не удалось получить поток для канала {ch.get('name', 'Без названия')}.")
    ...
    if links_found > 0:
        ...
        print(f"\nФайл playlist.m3u успешно сохранен и содержит {links_found} канал(ов).")
    else:
